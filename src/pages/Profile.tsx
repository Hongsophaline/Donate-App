      ...
      try {
        ...
        if (pRes.ok) {
          ...
          if (rRes.ok) {
            ...
            const incomingRequests = Array.isArray(requests)
              ? requests
              : requests.content || [];
            setPendingCount(
              incomingRequests.filter((r: any) => r.status === "PENDING")
                .length,
            );
          }
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
              ...
              <StatCard
                icon={
                  <Clock
                    size={22}
                    className={
                      pendingCount > 0 ? "text-orange-500 animate-pulse" : ""
                    }
                  />
                }
                value={pendingCount}
          ...
          <div className="mt-8 text-center">
            <p className="text-[10px] text-gray-300 font-mono uppercase tracking-widest">
              API Connection:{" "}
              {BASE_URL.includes("localhost") ? "Localhost" : "Remote"}
            </p>
          </div>
        </div>
      </div>
