...
const Browse: React.FC = () => {
  ...
  // Send Request - IMPROVED with better logging
  const handleSendRequest = async () => {
    ...
    try {
      ...
      console.log("Sending request with token:", token.substring(0, 20) + "..."); // Debug
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden">
            <div className="relative h-64">
              ...
              <button onClick={() => setIsDetailModalOpen(false)} className="absolute top-4 right-4 bg-black/20 p-2 rounded-full text-white hover:bg-black/40">
                <X size={20} />
              </button>
            </div>
            <div className="p-8">
              <div className="flex justify-between items-start mb-4">
                <h2 className="text-3xl font-black text-gray-900 uppercase">{selectedItem.title}</h2>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8 text-sm">
                <div className="bg-gray-50 p-3 rounded-xl">
                  <span className="block text-gray-400 uppercase text-xs font-black">Condition</span>
                  <span className="font-bold">{selectedItem.condition}</span>
                </div>
                <div className="bg-gray-50 p-3 rounded-xl">
                  <span className="block text-gray-400 uppercase text-xs font-black">Category</span>
              ...
              <button 
                ...
                className="w-full py-4 bg-[#C84C0E] text-white rounded-2xl font-black uppercase tracking-widest hover:bg-[#A33D0B]"
              >
                Send Request Now
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Item: <span className="font-bold text-gray-800">{selectedItem.title}</span>
            </p>
            <textarea
              className="w-full h-32 p-4 border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-green-500 outline-none resize-none mb-4"
